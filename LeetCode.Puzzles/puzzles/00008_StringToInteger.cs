...
public class StringToIntegerPuzzle : PuzzleBase
{
    public override void PrintOutput(int solutionIndex, params object[] parameters)
    {
        ...
        {
            1 => Invoke(() => { return MyAtoi_1(s); }),
            2 => Invoke(() => { return MyAtoi_2(s); }),
    ...
    {
        s = s.TrimStart();
        if (s.Length == 0 || char.IsLetter(s[0]) || s[0] == '.')
            return 0;
        ...
        ? (positive ? n : -n)
        : (positive ? int.MaxValue : int.MinValue);
    }
    ...
    public int MyAtoi_1(string s)
    {
        ...
        {
            if (char.IsLetter(s[i]) || s[i] == '.' || (state > 0 && !char.IsDigit(s[i])))
                break;
            ...
            if (positive && n >= max)
                return max;
            else if (!positive && -n <= min)
                return min;
            state = 1;
        }

        return (int)(positive ? n : -n);
    }
}